...
export function fallbackExtractBatch(rows) {
    const records = [];
    const skipped = [];
...
function pick(row, names) {
    const normalized = Object.entries(row).map(([key, value]) => [key.toLowerCase().replace(/[_-]/g, " ").trim(), value]);
...
function inferStatus(text) {
    ...
    if (/(closed|won|sold|sale done|deal closed)/.test(lower))
        return "SALE_DONE";
    if (/(not interested|junk|invalid|bad lead|remove)/.test(lower))
        return "BAD_LEAD";
    if (/(busy|not reachable|did not connect|no answer|no response)/.test(lower))
        return "DID_NOT_CONNECT";
    if (/(follow|new lead|interested|callback|call back|demo)/.test(lower))
...
function inferSource(text) {
    ...
    if (lower.includes("eden"))
        return "eden_park";
    if (lower.includes("varah"))
        return "varah_swamy";
    if (lower.includes("sarjapur"))
        return "sarjapur_plots";
    if (lower.includes("leads on demand"))
        return "leads_on_demand";
    return "";
}
function buildNotes(row, extraEmails, extraPhones) {
    ...
    if (extraPhones.length)
        notes.push(`Extra mobiles: ${extraPhones.join(", ")}`);
