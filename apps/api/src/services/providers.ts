...
export type BatchInput = Array<{ rowNumber: number; data: Record<string, string> }>;

let aiCircuitOpenUntil = 0;
let lastProviderUsedFallback = false;

export async function extractBatch(rows: BatchInput) {
  ...
  try {
    if (provider === "openai") return await extractWithOpenAi(rows);
    if (provider === "gemini") return await extractWithGemini(rows);
    if (provider === "anthropic" || provider === "claude") return await extractWithAnthropic(rows);
    return fallbackExtractBatch(rows);
  } catch (error) {
...
async function extractWithOpenAi(rows: BatchInput) {
  ...
  const completion = await client.chat.completions.create({
    ...
    ...(isOpenRouter ? {} : { response_format: { type: "json_object" as const } }),
    messages: [
      {
        role: "system",
        content:
          "Return only one valid JSON object. Do not include markdown, explanations, introductions, or code fences."
      },
...
function buildOpenAiCompatibleHeaders() {
  if (!process.env.OPENAI_BASE_URL?.includes("openrouter.ai")) return undefined;
  return {
...
async function extractWithGemini(rows: BatchInput) {
...
function generateGeminiContent(ai: GoogleGenAI, model: string, contents: string) {
...
async function extractWithAnthropic(rows: BatchInput) {
...
function getQuotaCooldownMs(error: unknown) {
  if (!(error instanceof Error)) return 60_000;
  const retryDelay = error.message.match(/retryDelay"?\s*:\s*"?(\d+)s/i);
  if (retryDelay?.[1]) return Number(retryDelay[1]) * 1000;
