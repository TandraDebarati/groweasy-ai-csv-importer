import { CRM_FIELDS, CRM_STATUSES, DATA_SOURCES, type CrmField, type CrmRecord, type RecordQuality } from "@groweasy/shared";
...
const weightedFields: CrmField[] = ["name", "email", "mobile_without_country_code", "created_at", "crm_status", "data_source", "city", "state"];
...
export function normalizeRecord(record: CrmRecord): CrmRecord {
  const normalized: CrmRecord = {
    ...
    crm_status: CRM_STATUSES.includes(record.crm_status as never) ? record.crm_status : "",
    crm_note: clean(record.crm_note).replace(/\r?\n/g, "\\n"),
    data_source: DATA_SOURCES.includes(record.data_source as never) ? record.data_source : "",
...
export function assessRecord(record: CrmRecord, rowNumber: number): RecordQuality {
  ...
  if (!record.email) warnings.push("Missing email");
  if (!record.mobile_without_country_code) warnings.push("Missing mobile");
  if (!record.crm_status) warnings.push("Status could not be confidently mapped");
  if (!record.data_source) warnings.push("Data source not confidently matched");
  if (record.created_at && Number.isNaN(new Date(record.created_at).getTime())) warnings.push("Date may need review");
...
function applyLocationCleanup(record: CrmRecord) {
