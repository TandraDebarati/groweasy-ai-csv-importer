import type { ImportResponse, SkippedRecord } from "@groweasy/shared";
...
export async function importRows(rows: RawRow[]): Promise<ImportResponse> {
  ...
  const quality: ImportResponse["quality"] = [];
  ...
  const averageCompleteness =
    quality.length > 0 ? Math.round(quality.reduce((sum, item) => sum + item.completeness, 0) / quality.length) : 0;
  const runtime = getProviderRuntime();
